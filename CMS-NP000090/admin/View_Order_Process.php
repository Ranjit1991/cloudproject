...
<div class="navbar ">
 ...
 <div  class="show_table">
  <center><h3>View Orders Process </h3></center>
<?php
 //contion establish
 ...
 if(mysqli_connect_error($conn))
 {
  ...
  $query = "select * from cargo_orders where Order_Status='Process'";
  $result = mysqli_query($conn,$query);
  static $row_num = 0;

  if(mysqli_num_rows($result)>0){

    echo'<center><table><tr><th>Order ID</th><th>Departure</th><th>Arrival</th><th>Sender Name</th><th>Sender Phone</th><th>Receiver Name</th><th>Receiver Phone</th><th>Container Model</th><th>Function</th></tr>';
      // output data of each row
      while($row = mysqli_fetch_assoc($result)){
          $row_num++;
          echo'<tr class="thead-light"><td>'.$row["Order_ID"].'</td><td>'.$row["Departure_Name"].'</td><td>'.$row["Arrival_Name"].'</td><td>'.$row["Sender_Name"].'</td><td>'.$row["Sender_Phone"].'</td><td>'.$row["Receiver_Name"].'</td><td>'.$row["Receiver_Phone"].'</td><td>'.$row["Container_Model"].'</td><td><a  class="accept_btn" href="../functions/Order_Accept.php?id='.$row["Container_Model"].'">Accept</a> <a  class="delete_btn" href="../functions/Order_Reject.php?id='.$row["Container_Model"].'">Reject</a></td></tr>';}
      echo'</center></table>';
