...
<?php
  ...
  if(mysqli_num_rows($result)>0){
    ...
    echo'<center><table ><tr><th>No.</th><th>Container ID</th><th>Container Model</th><th>Container Description</th><th>Container Status</th><th>Function</th></tr>';
      // output data of each row
      while($row = mysqli_fetch_assoc($result)){
          $row_num++;
          echo'<tr> <td>'.$row_num.'</td><td>'.$row["Container_ID"].'</td><td>'.$row["Container_Model"].'</td><td>'.$row["Container_Description"].'</td>
		  <td>'.$row["Container_Status"].'</td>
		  <td><a  class="delete_btn" href="../functions/Container_Delete.php?id='.$row["Container_ID"].'">Delete</a>
		  <a  class="accept_btn" href="../admin/update_container.php?id='.$row["Container_ID"].'">Update</a>
		  </td></tr>';}
      echo'</center></table>';
